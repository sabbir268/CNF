...
SELECT bill.bill_id as 'bill_id', bill.bill_no as 'bill_no' , bill.bill_date as 'bill_date', job.job_no as 'job_no',job.customer_id as 'customer_id' ,messers_to_bill(bill.bill_id) as 'messers',address_to_bill(bill.bill_id) as 'address', bill_to_lc(bill.bill_id) as 'bill_lc' ,job.ac as 'ac',bill.accesable_value as 'accesable_value' , bill.comission_acc as 'comission_acc' , ((bill.accesable_value*bill.comission_acc)/100)  as 'net_comission_am' ,total_bill_am(bill.bill_id) as 'total_bill' , bill.discount as 'discount' , (total_bill_am(bill.bill_id)-bill.discount) as 'sub_total', bill.paid_am as 'paid_am' ,((total_bill_am(bill.bill_id)-bill.discount)-bill.paid_am) as 'due' , bill_incomplete_status(bill.bill_id) as 'status' , bill_to_goods(bill.bill_id) as 'desc', job.port_id as 'port_id'
